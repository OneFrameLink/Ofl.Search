...
namespace Ofl.Search
{
    public static class IndexManagerExtensions
    {
            ...
            where T : class
        {
            ...
            // Get the index.
            IIndex index = await indexManager.GetIndexAsync(indexName, cancellationToken).ConfigureAwait(false);

            // Get the write operations.
            IIndexWriteOperations<T> wo = await index.GetWriteOperationsAsync<T>(cancellationToken).ConfigureAwait(false);
            ...
            where T : class
        {
            ...
            IIndexReadOperations<T> ro = await index.GetReadOperationsAsync<T>(cancellationToken).ConfigureAwait(false);
